...
save_to_db.py
-------------
Sauvegarde les offres Adzuna dans PostgreSQL et MinIO.
Même pattern que France Travail et Remotive.
"""
...
def sauvegarder_brut_dans_minio(offres_brutes, source="adzuna"):
    try:
...
def inserer_dans_postgres(offres_propres):
    ...
    cursor.execute("SELECT COUNT(*) FROM job_offers WHERE source = 'adzuna'")
    total_adzuna = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM job_offers")
    total = cursor.fetchone()[0]

    print(f"   → Offres Adzuna en base : {total_adzuna}")
