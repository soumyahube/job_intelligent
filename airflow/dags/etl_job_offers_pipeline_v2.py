...
def remotive_archive(**context):
    ...
    mod    = load_module("remotive_save", PATHS["remotive"]["save"])
...
def adzuna_archive(**context):
    ...
    mod    = load_module("adzuna_save", PATHS["adzuna"]["save"])
    offres = context["ti"].xcom_pull(task_ids="adzuna.adzuna_extract", key="offres_brutes")
    mod.sauvegarder_brut_dans_minio(offres, source="adzuna")
...
with DAG(
    dag_id="etl_job_offers_pipeline_v2",
